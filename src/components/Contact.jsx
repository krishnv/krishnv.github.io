...
export default function Contact({ contact = {}, name }) {
  const links = [
    ...
    contact.email && { icon: MailIcon, label: 'Email', href: `mailto:${contact.email}` },
  ].filter(Boolean)
  ...
  return (
    <section id="contact" className="py-20 px-6 max-w-5xl mx-auto">
        ...
        <div className="flex flex-wrap gap-4">
          {links.map(({ icon: Icon, label, href }) => (
            <a
              ...
              className="flex items-center gap-2 px-4 py-2.5 border border-gray-200 rounded-lg text-sm text-gray-600 hover:text-gray-900 hover:border-gray-400 transition-all"
            >
              <Icon />
              {label}
            </a>
      ...
      <p className="text-xs text-gray-400 mt-16">© {new Date().getFullYear()} {name}</p>
    </section>
  )
}
